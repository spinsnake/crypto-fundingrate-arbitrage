...
## What it does
- Scans funding rates on Asterdex and Hyperliquid, normalizes to the max interval, filters by volume/delist/price-edge, and ranks spreads (`src/strategies/funding_arb.py`).
...
## Key logic notes
- Strategy normalizes funding to the max interval, subtracts fee+slippage (using config percents / 100 in code) and enforces:
